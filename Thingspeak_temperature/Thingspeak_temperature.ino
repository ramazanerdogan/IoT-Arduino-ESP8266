...
void loop() {
  ...
  sicaklik_olc();
  int x = ThingSpeak.writeField(SECRET_CH_ID, 1, sicaklik, SECRET_WRITE_APIKEY); //alan1'e sicaklik yaz
  if(x == 200){
  ...
  //nem yazdiracagiz
   x = ThingSpeak.writeField(SECRET_CH_ID, 2, nem, SECRET_WRITE_APIKEY); //alan2'e nem yaz
  if(x == 200){
    Serial.println("Kanala veri yazildi");
  }
  else{
